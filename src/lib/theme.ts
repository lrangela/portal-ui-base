...
function normalizeFeatures(value: unknown, fallback: FeatureItem[]) {
  ...
  const normalized = value
    .filter((item) => item && typeof item === 'object')
    .map((item) => ({
      icon: normalizeString((item as FeatureItem).icon, 'Sparkles') as FeatureIconName,
...
export function globalsToTheme(globals: Record<string, unknown> = {}): ThemeConfig {
  ...
  const backgroundType = normalizeString(globals.backgroundType, theme.background.type) as BackgroundType;
